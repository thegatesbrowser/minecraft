...
func add_new_portal(id:Vector3):
	
	send_to_server.rpc_id(1,id)
	

	#portal_url.id = id
	#UI.add_child(portal_url)
	#
func check_portal(id:Vector3):
	server_check_portal.rpc_id(1,id)
	
@rpc("any_peer","call_local")
func send_to_server(id:Vector3,url := "https://thegates.io/worlds/world.gate"):
	server_info[id] = {
		"url": url
	}
	print(server_info[id])
	
func add_portal_url(id:Vector3,url:String):
	send_to_server.rpc_id(1,id,url)
	
@rpc("any_peer","call_local")
func server_check_portal(id:Vector3):
...
@rpc("any_peer","call_local")
func give_client(info:Dictionary):
